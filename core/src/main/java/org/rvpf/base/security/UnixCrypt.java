 ...
 *
 * @version $Id: UnixCrypt.java 2764 2015-09-13 19:56:28Z SFB $
...
public final class UnixCrypt
{
     ...
     */
    private static int toSixBit(final int num)
    {
     ...
     * code, each having 6 bits.
     */
    private static long toSixBit(final long num)
    {
     ...
     */
    private static long[] desSetkey(final long keyword)
    {
     ...
     * environment.
     */
    private static long desCipher(final long in, int salt, int numIter, final long[] ks)
    {
        salt = toSixBit(salt);
        long l = in;
        long r = l;
     ...
     */
    private static void initPerm(final long[][] perm, final byte[] p, final int charsOut)
    {
     ...
     */
    public static String crypt(final char[] key, final String setting)
    {
        final long constdatablock = 0L; /* encryption constant */
        final byte[] cryptresult = new byte[13]; /* encrypted result */
        long keyword = 0L;
        /* invalid parameters! */
        ...
        final long[] ks = desSetkey(keyword);

        int salt = 0;
        for (int i = 2; --i >= 0;)
        {
            final char c = (i < setting.length()) ? setting.charAt(i) : '.';
        ...
        try {
            return new String(cryptresult, 0, 13, "US-ASCII");
    ...
    public static void main(final String[] arg)
    {
        ...
        System.err.println("Crypt=" + crypt(arg[0].toCharArray(), arg[1]));
    }
