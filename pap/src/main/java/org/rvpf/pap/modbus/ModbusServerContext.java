 ...
 *
 * $Id: ModbusServerContext.java 4076 2019-06-11 17:09:30Z SFB $
 */
    ...
    extends ModbusContext
{
    ...
    {
        final String originName = origin.getNameInUpperCase().get();

        if (_origins.isEmpty() || _origins.containsKey(originName)) {
            return _origins.put(originName, origin) == null;
        }
            ...
            final Attributes pointAttributes)
    {
        ...
        if (!origin.isPresent()
                || (!point.getAttributes(
                    Modbus.ATTRIBUTES_USAGE).isPresent())) {
            return false;
        }
     ...
     */
    @Override
    protected ModbusClientProxy newRemoteProxy(final Origin origin)
    {
