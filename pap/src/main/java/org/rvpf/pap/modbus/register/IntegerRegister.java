 ...
 *
 * $Id: IntegerRegister.java 3961 2019-05-06 20:14:59Z SFB $
 */
    ...
    extends DoubleWordRegister
{
    ...
    public PointValue[] getPointValues()
    {
        ...
        if (getPoints() != NO_POINTS) {
            ...
            if (_signed) {
                value = Integer.valueOf(getInt());
            } else {
                value = Long.valueOf(getInt() & 0xFFFFFFFFL);
            }
    ...
    /**
     * Gets the int value from the words.
     *
    ...
    protected int getInt()
    {
        ...
        if (isMiddleEndian()) {
            intValue = (getContent() & 0xFFFF)
                       + (getNextRegister().getContent() << 16);
        } else {
            intValue = (getContent() << 16)
                       + (getNextRegister().getContent() & 0xFFFF);
        }
    ...
    /**
     * Sets the int value to the words.
     *
     * @param intValue The int value.
     */
    protected void setInt(final int intValue)
    {
        if (isMiddleEndian()) {
            setContent((short) intValue);
            getNextRegister().setContent((short) (intValue >> 16));
        } else {
            setContent((short) (intValue >> 16));
            getNextRegister().setContent((short) intValue);
        }
    }
